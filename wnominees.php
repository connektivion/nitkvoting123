...
{
	$query="SELECT `id`, `firstname`, `surname`, `roll`, `votes` FROM `student_info` WHERE `branch`='$branch' AND `year`='$year' and `votes`='1'";
	$query_run=mysql_query($query);
	if(mysql_num_rows($query_run)==0)
		echo "No Nominees yet!<br>Check final list<br>";
	else
	 { 
	 	...
	 	while($var=mysql_fetch_assoc($query_run))
	 		{
	 			...
	 			if($givevotes==1)
	   				{
	   					echo "<th><form action='wnominees.php' method='post'>
